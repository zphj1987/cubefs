...
import io.chubao.fs.CfsLibrary.StatInfo;
...
public class CFSDriverIns {
    private final static int ATTR_MODE = 1;
    ...
    private static final Logger log = LoggerFactory.getLogger(CFSDriverIns.class);
    ...
    public void truncate(String path, long newLength) throws CFSException {
        ...
        stat.size = newLength;
        int st = driver.cfs_setattr_by_path(this.clientID, path, stat, ATTR_SIZE);
        if (StatusCodes.get(st) != StatusCodes.CFS_STATUS_OK) {
    ...
    public void chmod(String path, int mode) throws CFSException {
        ...
        CfsLibrary.StatInfo stat = new CfsLibrary.StatInfo();
        stat.mode = mode;
        int st = driver.cfs_setattr_by_path(this.clientID, path, stat, ATTR_MODE);
        if (StatusCodes.get(st) != StatusCodes.CFS_STATUS_OK) {
    ...
    public int list(int fd, ArrayList<CFSStatInfo> fileStats) throws CFSException {
        ...
        long[] inodeIds = new long[count];
        Map<Long, String> names = new HashMap<Long, String>(count);
        for (int i = 0; i < count; i++) {
            dents[i].read();
            inodeIds[i] = dents[i].ino;
            names.put(dents[i].ino, new String(dents[i].name, 0, dents[i].nameLen, StandardCharsets.UTF_8));
        }
        ...
        int num = driver.cfs_batch_get_inodes(this.clientID, fd, inodeIds, statSlice, count);
    ...
    private void verifyPath(String path) throws CFSException {
        if (path == null || path.trim().length() == 0) {
            throw new CFSNullArgumentException("path is invalid.");
        }
